...
- MR/PR references resolve only against configured watched repositories.
  ...
  repositories (`ls-remote`/`fetch`/`archive`/`diff` on provider-owned refs)
  into temporary snapshots that are never persisted. Unsupported, ambiguous,
  or unresolved references fail honestly and never fall back to auditing the
