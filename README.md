...
- Controller-side MR fetching is read-only (`ls-remote`, `fetch`, `archive`,
  `diff` on provider-owned refs) and limited to the configured repository's
  `origin`. It verifies the base/head relationship and materializes temporary
  read-only base/head snapshots.
